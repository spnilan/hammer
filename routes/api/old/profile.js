...
router.get('/', async (req, res) => {
    try {
        ...
        if (!profiles) {
            return res.status(400).json({ msg: 'No profiles found for user'})
        }
...
router.get('/github/:username', (req, res) => {
    const [username, githubId, githubSecret] = [
        req.params.username,
    ...
    try {
        const options = {
            uri: `https://api.github.com/users/${username}/repos?per_page=5&sort=created:asc&client_id=${githubId}&client_secret=${githubSecret}`,
            method: 'GET',
